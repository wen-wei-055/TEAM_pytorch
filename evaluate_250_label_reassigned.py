...
def reassigned_label(st_pga_times, st_coords):  # (stations, 7) , (stations,3)
...
def calculate_warning_times(config, model_list, data, event_metadata, batch_size, sampling_rate=100,
    ...
    for i, _ in tqdm(enumerate(event_metadata.iterrows()), total=len(event_metadata)): #分別進入每個事件
        ...
        pga_times_true = reassigned_label(pga_times_true_pre, coords)
...
if __name__ == '__main__':
    ...
    nearest_topK_stations = json.load(open('dataset_configs/nearest_top3_stations.json','r'))
    ...
    if args.val:
        ...
        training_params['data_path'] = "/mnt/disk5/william/Dataset/TEAM/taiwan/CWBSN/PGA_location_oriented/val_CWBSN_window_split_year.hdf5"
        test_set = False
    else:
        output_dir = os.path.join(args.experiment_path, f'evaluation{suffix}', 'test')
        training_params['data_path'] = "/mnt/disk5/william/Dataset/TEAM/taiwan/CWBSN/PGA_location_oriented/test_CWBSN_window_split_year.hdf5"
        test_set = True
